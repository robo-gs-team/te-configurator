...
async function fetchConfigurator(
  ...
  try {
    ...
    return {
      configurator: null,
      error:
        "No configurator linked to this product. Add the product ID in the app admin, enable Active, and click Save changes.",
    };
  } catch (err) {
...
async function openConfigurator(productId: string, trigger: HTMLElement) {
  ...
  try {
    ...
    if (!configurator) {
      showConfigureError(
        trigger,
        "No configurator is linked to this product. Add this product's numeric ID in the app admin, enable Active, and click Save changes.",
      );
      return;
    }
